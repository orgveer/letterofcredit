...
function clearContainers() {
  CONTAINER_IDS=$(docker ps -a | awk '($2 ~ /dev-peer.*.letterofcreditcc.*/) {print $1}')
...
function removeUnwantedImages() {
  DOCKER_IMAGE_IDS=$(docker images | awk '($1 ~ /dev-peer.*.letterofcreditcc.*/) {print $3}')
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/buyer.letterofcredit.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/seller.letterofcredit.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml

  cd crypto-config/peerOrganizations/buyerbank.letterofcredit.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA3_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml

  cd crypto-config/peerOrganizations/sellerbank.letterofcredit.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA4_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml

  cd crypto-config/peerOrganizations/port.letterofcredit.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile FourOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/BuyerMSPanchors.tx -channelID $CHANNEL_NAME -asOrg BuyerMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for BuyerBankMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile FourOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/BuyerBankMSPanchors.tx -channelID $CHANNEL_NAME -asOrg BuyerBankMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for BuyerBankMSP..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for SellerBankMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile FourOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/SellerBankMSPanchors.tx -channelID $CHANNEL_NAME -asOrg SellerBankMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for SellerBankMSP..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for PortMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile FourOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/PortMSPanchors.tx -channelID $CHANNEL_NAME -asOrg PortMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for PortMSP..."
    exit 1
  fi
}
